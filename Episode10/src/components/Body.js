    ...
    return(
        <div className='w-full relative top-20 px-28 py-8'>
            ...
            <div className="w-full flex flex-wrap gap-5">
                {filterdRestaurant.map((x)=><Link className='w-[21%] mb-6 text-[#222] rounded-xl hover:scale-90 transition-all' to={"/restaurant/"+x.info.id}><RestaurantCard key={x.info.id} resData={x}/></Link>)}
            </div>
        </div>
    )
}
